const GOOGLE_API_URL = "https://script.google.com/macros/s/AKfycbyp3xb5dI_lRcUqEHL9CVycsmlqm94YbcmaBc7kfwcD4Yl9zkckl3R6BKwDuuJ5s7ct/exec";
let TARGET_SHEET = "個人記帳", PAGE_MODE = "PERSONAL", currentType = '支出', systemBalances = {};

const categoryConfigs = {
    'PERSONAL': { '支出': { '生活支出': ['早餐/午餐/晚餐', '點心/宵夜/飲料', '娛樂費', '交通費', '線上購物', '實體購物'], '固定支出': ['房租', '電信費', '瓦斯費', '債務還款', 'APP訂閱', '水費/電費', '保險(大人)'], '小孩支出': ['尿布', '健康', '娛樂', '學雜費', '保險(小孩)'] }, '收入': { '個人收入': ['薪資', '一般', '其他'] } },
    'GROUP': { '支出': { '選品支出 (零售)': ['手續費', '選品進貨', '藍新沖銷', '國際運費', '國內運費', '斷貨退款'], '代理支出 (批發)': ['成本', '代墊款', '斷貨退款', '國際運費', '國內運費', '批發儲值(台幣)'], '批發網(韓幣)': ['代理下單'] }, '收入': { '選品收入 (零售)': ['現貨出清', '記事本收款', 'EasyStore 訂單'], '代理收入 (批發)': ['代墊款回收', '運費收回'], '批發網(韓幣)': ['儲值'] } },
    'DESSERT': { '支出': { '甜點支出': ['材料進貨', '包材進貨', '運費', '手續費', '場地費'] }, '收入': { '甜點收入': ['門市', '私訊', '表單', 'PAYUNi 入帳'], '教學收入': ['師資', '小幫手'] } }
};
...
async function updateWaterBalance() {
    ...
    document.getElementById('quickAmount').value = ''; fetchDashboard();
}
...
function updateAccounts() {
    ...
    else if (cat.includes('批發')) list = ['批發網點數'];
...
document.getElementById('form').addEventListener('submit', async function(e) {
    ...
    const send = async (a, n, t=currentType, c=document.getElementById('category').value) => {
        await fetch(`${GOOGLE_API_URL}?action=add&targetSheet=${encodeURIComponent(TARGET_SHEET)}&date=${document.getElementById('date').value}&type=${t}&subType=${document.getElementById('subType').value}&category=${c}&account=${document.getElementById('account').value}&amount=${a}&note=${encodeURIComponent(n)}`);
    };
    try {
        if (document.getElementById('category').value === 'EasyStore 訂單' && document.getElementById('account').value === '藍新待撥款') {
            await send(amt, note); await send(Math.round(amt*0.028), '(2.8%手續費)', '支出', '手續費');
        } else { await send(amt, note); }
        alert("記帳成功！"); fetchDashboard();
        document.getElementById('amount').value = ''; document.getElementById('note').value = '';
    } catch (e) { alert("連線失敗"); } finally { btn.disabled = false; }
});
