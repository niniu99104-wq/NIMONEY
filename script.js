...
const categoryConfigs = {
    'PERSONAL': { '支出': { '變動支出': ['早餐/午餐/晚餐', '點心/宵夜/飲料', '娛樂費', '交通費', '線上購物', '實體購物'], '固定支出': ['房租', '電信費', '瓦斯費', '債務還款', 'APP訂閱', '水費/電費', '保險(大人)'], '小孩支出': ['尿布', '健康', '娛樂', '學雜費', '保險(小孩)'] }, '收入': { '個人收入': ['薪資', '一般', '其他'] } },
    ...
    'DESSERT': { '支出': { '甜點支出': ['材料進貨', '包材進貨', '運費', '手續費', '場地費', '教具購買'] }, '收入': { '甜點收入': ['實體門市', '私訊訂購', '表單訂購', 'PAYUNi 入帳'], '教學收入': ['沐光毓師資', '沐光毓小幫手'] } }
};
...
document.getElementById('form').addEventListener('submit', async function(e) {
    ...
    try {
        if (catValue === 'EasyStore 訂單' && accValue === '藍新待撥款') {
            await send(amt, note); await send(Math.round(amt * 0.028), '(2.8%手續費)', '支出', '選品支出 (零售)', '手續費');
